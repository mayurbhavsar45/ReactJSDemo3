...
JSHG.init = function (args) {
    ...
    for (var arg in args) {
        if (arg in JSHG.INIT_PARAMETERS && typeof (args[arg]) != JSHG.INIT_PARAMETERS[arg]) {

        }
            //throw "the typeof " + arg + "is incorrect. \
            //       It must be " + JSHG.INIT_PARAMETERS[arg] + " instead of " + typeof (args[arg]);
    }

    JSHG.resetVariables_();

    // actionCallback is compulsory
    if (!("actionCallback" in args))
        throw "actionCallback is required!";
    JSHG.actionCallback_ = args["actionCallback"];
    if ("learnCallback" in args)
        JSHG.learnCallback_ = args["learnCallback"];
    if ("learnDivId" in args)
        JSHG.learningArea_ = $("#" + args["learnDivId"]);
    if ("gestureDivId" in args)
        JSHG.gestureArea_ = $("#" + args["gestureDivId"]);
    ...
    JSHG.initWorker_(args["workerConfig"]);
